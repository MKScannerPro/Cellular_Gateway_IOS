...
@implementation MKCKScanInfoCellModel
@end

@interface MKCKScanInfoCell ()
...
@implementation MKCKScanInfoCell

+ (MKCKScanInfoCell *)initCellWithTableView:(UITableView *)tableView {
    MKCKScanInfoCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCKScanInfoCellIdenty"];
    if (!cell) {
        cell = [[MKCKScanInfoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCKScanInfoCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ck_connectButtonPressed:)]) {
        [self.delegate ck_connectButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCKScanInfoCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCKScanInfoCellModel.class]) {
        return;
    }
    //顶部
...
- (UIImageView *)rssiIcon {
    if (!_rssiIcon) {
        ...
        _rssiIcon.image = LOADICON(@"MKGatewayFour", @"MKCKScanInfoCell", @"ck_scan_rssiIcon.png");
    }
    return _rssiIcon;
}
