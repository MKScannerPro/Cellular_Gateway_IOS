...
@interface MKCKBXPSPayloadController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKBXPSPayloadModel *dataModel;

@end

@implementation MKCKBXPSPayloadController

- (void)dealloc {
    NSLog(@"MKCKBXPSPayloadController销毁");
}
...
- (void)loadSection1Datas {
    ...
    cellModel8.index = 9;
    cellModel8.msg = @"Temperature and Humidity";
    cellModel8.isOn = self.dataModel.TH;
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"BXP-Sensor payload";
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKBXPSPayloadController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKBXPSPayloadModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKBXPSPayloadModel alloc] init];
    }
    return _dataModel;
}
