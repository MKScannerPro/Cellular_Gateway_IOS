...
@interface MKCKThPayloadController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKThPayloadModel *dataModel;

@end

@implementation MKCKThPayloadController

- (void)dealloc {
    NSLog(@"MKCKThPayloadController销毁");
}
...
- (void)loadSubViews {
    ...
    self.defaultTitle = @"BXP-T&H payload";
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKThPayloadController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKThPayloadModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKThPayloadModel alloc] init];
    }
    return _dataModel;
}
