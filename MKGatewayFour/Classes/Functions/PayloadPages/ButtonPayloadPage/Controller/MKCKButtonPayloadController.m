...
@interface MKCKButtonPayloadController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKButtonPayloadModel *dataModel;

@end

@implementation MKCKButtonPayloadController

- (void)dealloc {
    NSLog(@"MKCKButtonPayloadController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"BXP-Button payload";
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKButtonPayloadController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKButtonPayloadModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKButtonPayloadModel alloc] init];
    }
    return _dataModel;
}
