...
@interface MKCKTlmPayloadController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKTlmPayloadModel *dataModel;

@end

@implementation MKCKTlmPayloadController

- (void)dealloc {
    NSLog(@"MKCKTlmPayloadController销毁");
}
...
- (void)mk_textSwitchCellStatusChanged:(BOOL)isOn index:(NSInteger)index {
    ...
    if (index == 2) {
        //TLM version
        self.dataModel.tlmVersion = isOn;
...
- (void)loadSection1Datas {
    MKTextSwitchCellModel *cellModel1 = [[MKTextSwitchCellModel alloc] init];
    cellModel1.index = 2;
    cellModel1.msg = @"TLM version";
    cellModel1.isOn = self.dataModel.tlmVersion;
    [self.section1List addObject:cellModel1];
    
    MKTextSwitchCellModel *cellModel2 = [[MKTextSwitchCellModel alloc] init];
    cellModel2.index = 3;
    cellModel2.msg = @"Battery voltage";
    cellModel2.isOn = self.dataModel.voltage;
    [self.section1List addObject:cellModel2];
    
    MKTextSwitchCellModel *cellModel3 = [[MKTextSwitchCellModel alloc] init];
    cellModel3.index = 4;
    cellModel3.msg = @"Chip temperature";
    cellModel3.isOn = self.dataModel.temperature;
    [self.section1List addObject:cellModel3];
    
    MKTextSwitchCellModel *cellModel4 = [[MKTextSwitchCellModel alloc] init];
    cellModel4.index = 5;
    cellModel4.msg = @"ADV count";
    cellModel4.isOn = self.dataModel.advCount;
    [self.section1List addObject:cellModel4];
    
    MKTextSwitchCellModel *cellModel5 = [[MKTextSwitchCellModel alloc] init];
    cellModel5.index = 6;
    cellModel5.msg = @"SEC count";
    cellModel5.isOn = self.dataModel.secCount;
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Eddystone-TLM payload";
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKTlmPayloadController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKTlmPayloadModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKTlmPayloadModel alloc] init];
    }
    return _dataModel;
}
