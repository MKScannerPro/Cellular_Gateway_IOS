...
@implementation MKCKSyncDeviceCellModel
@end

@interface MKCKSyncDeviceCell ()
...
@implementation MKCKSyncDeviceCell

+ (MKCKSyncDeviceCell *)initCellWithTableView:(UITableView *)tableView {
    MKCKSyncDeviceCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCKSyncDeviceCellIdenty"];
    if (!cell) {
        cell = [[MKCKSyncDeviceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCKSyncDeviceCellIdenty"];
    }
    return cell;
}
...
- (void)backBtnPressed {
    ...
    [self updateIcon];
    if ([self.delegate respondsToSelector:@selector(ck_syncDeviceCell_selected:index:)]) {
        [self.delegate ck_syncDeviceCell_selected:self.backBtn.selected index:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCKSyncDeviceCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCKSyncDeviceCellModel.class]) {
        return;
    }
...
- (void)updateIcon {
    UIImage *btnIcon = (self.backBtn.selected ? LOADICON(@"MKGatewayFour", @"MKCKSyncDeviceCell", @"ck_listButtonSelectedIcon.png") : LOADICON(@"MKGatewayFour", @"MKCKSyncDeviceCell", @"ck_listButtonUnselectedIcon.png"));
