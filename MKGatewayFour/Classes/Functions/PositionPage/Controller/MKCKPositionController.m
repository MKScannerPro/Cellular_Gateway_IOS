...
@implementation MKCKPositionController

- (void)dealloc {
    NSLog(@"MKCKPositionController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Fix mode
        MKCKFixModeController *vc = [[MKCKFixModeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //GPS parameters
        MKCKGpsFixController *vc = [[MKCKGpsFixController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //3-Axis parameters
        MKCKAxisSettingsController *vc = [[MKCKAxisSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)loadSectionDatas {
    ...
    cellModel1.leftMsg = @"Fix mode";
    cellModel1.showRightIcon = YES;
    [self.dataList addObject:cellModel1];
    
    MKNormalTextCellModel *cellModel2 = [[MKNormalTextCellModel alloc] init];
    cellModel2.leftMsg = @"GPS parameters";
