...
@interface MKCKMqttSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCKServerConfigDeviceFooterViewDelegate,
MKCAFileSelectControllerDelegate,
MFMailComposeViewControllerDelegate,
MKCKImportServerControllerDelegate>
...
@property (nonatomic, strong)MKCKMqttSettingsModel *dataModel;

@property (nonatomic, strong)MKCKServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKCKServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCKMqttSettingsController

- (void)dealloc {
    NSLog(@"MKCKMqttSettingsController销毁");
}
...
#pragma mark - MKCKServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)ck_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    ...
    if (index == 1) {
        //Import Config File
        MKCKImportServerController *vc = [[MKCKImportServerController alloc] init];
        vc.delegate = self;
        [self.navigationController pushViewController:vc animated:YES];
    }
    if (index == 2) {
...
#pragma mark - MKCKImportServerControllerDelegate
- (void)ck_selectedServerParams:(NSString *)fileName {
    [MKCKExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCKMqttSettingsModel *model = [MKCKMqttSettingsModel mk_modelWithJSON:returnData];
...
//    }
    ...
    [MKCKExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)showClearAlertView {
    ...
    [alertView showAlertWithTitle:@"" message:msg notificationName:@"mk_ck_needDismissAlert"];
}

- (void)clearAllParams {
    MKCKMqttSettingsModel *emptyModel = [[MKCKMqttSettingsModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKMqttSettingsController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKMqttSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKMqttSettingsModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCKServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCKServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 490.f)];
...
- (MKCKServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCKServerConfigDeviceFooterViewModel alloc] init];
    }
