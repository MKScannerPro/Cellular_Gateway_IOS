...
@implementation MKCKMQTTSSLForDeviceViewModel
@end

@interface MKCKMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCKMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(ck_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate ck_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(ck_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate ck_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(ck_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate ck_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(ck_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate ck_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ck_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate ck_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCKMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCKMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayFour", @"MKCKMQTTSSLForDeviceView", @"ck_switchSelectedIcon.png") : LOADICON(@"MKGatewayFour", @"MKCKMQTTSSLForDeviceView", @"ck_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayFour", @"MKCKMQTTSSLForDeviceView", @"ck_switchUnselectedIcon.png") forState:UIControlStateNormal];
