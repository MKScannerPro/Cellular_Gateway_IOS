//
//  MKCKServerConfigDeviceSettingView.m
...
@interface MKCKServerConfigDeviceSettingView ()
...
@implementation MKCKServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ck_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate ck_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ck_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate ck_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ck_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate ck_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
