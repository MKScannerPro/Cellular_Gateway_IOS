//
//  MKCKFilterByAdvNameController.m
...
@interface MKCKFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKFilterByAdvNameModel *dataModel;

@end

@implementation MKCKFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKCKFilterByAdvNameController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKFilterByAdvNameController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
