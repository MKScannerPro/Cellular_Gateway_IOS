...
@interface MKCKFilterByUIDController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKFilterByUIDModel *dataModel;

@end

@implementation MKCKFilterByUIDController

- (void)dealloc {
    NSLog(@"MKCKFilterByUIDController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKFilterByUIDController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKFilterByUIDModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKFilterByUIDModel alloc] init];
    }
    return _dataModel;
}
