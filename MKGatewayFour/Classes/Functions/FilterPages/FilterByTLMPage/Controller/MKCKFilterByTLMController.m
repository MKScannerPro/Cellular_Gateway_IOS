...
@interface MKCKFilterByTLMController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKFilterByTLMModel *dataModel;

@end

@implementation MKCKFilterByTLMController

- (void)dealloc {
    NSLog(@"MKCKFilterByTLMController销毁");
}
...
- (void)configFilterStatus:(BOOL)isOn {
    ...
    [MKCKInterface ck_configFilterByTLMStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configTLMVersion:(NSInteger)version {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCKInterface ck_configFilterByTLMVersion:version sucBlock:^{
        [[MKHudManager share] hide];
...
- (MKCKFilterByTLMModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKFilterByTLMModel alloc] init];
    }
    return _dataModel;
}
