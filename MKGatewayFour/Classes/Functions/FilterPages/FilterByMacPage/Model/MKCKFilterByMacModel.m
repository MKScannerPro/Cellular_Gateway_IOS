...
@interface MKCKFilterByMacModel ()
...
@implementation MKCKFilterByMacModel
...
- (BOOL)readFilterPreciseMatch {
    ...
    [MKCKInterface ck_readFilterByMacPreciseMatchWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterPreciseMatch {
    __block BOOL success = NO;
    [MKCKInterface ck_configFilterByMacPreciseMatch:self.match sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReverseFilter {
    __block BOOL success = NO;
    [MKCKInterface ck_readFilterByMacReverseFilterWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReverseFilter {
    __block BOOL success = NO;
    [MKCKInterface ck_configFilterByMacReverseFilter:self.filter sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readMacList {
    __block BOOL success = NO;
    [MKCKInterface ck_readFilterMACAddressListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configMacList:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKCKInterface ck_configFilterMACAddressList:list sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
