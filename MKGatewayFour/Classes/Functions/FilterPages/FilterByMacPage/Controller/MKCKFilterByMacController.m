...
@interface MKCKFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKFilterByMacModel *dataModel;

@end

@implementation MKCKFilterByMacController

- (void)dealloc {
    NSLog(@"MKCKFilterByMacController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKFilterByMacController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKFilterByMacModel alloc] init];
    }
    return _dataModel;
}
