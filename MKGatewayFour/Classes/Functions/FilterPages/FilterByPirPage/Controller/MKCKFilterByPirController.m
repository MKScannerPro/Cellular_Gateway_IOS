...
@interface MKCKFilterByPirController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKFilterByPirModel *dataModel;

@end

@implementation MKCKFilterByPirController

- (void)dealloc {
    NSLog(@"MKCKFilterByPirController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKFilterByPirController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKFilterByPirModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKFilterByPirModel alloc] init];
    }
    return _dataModel;
}
