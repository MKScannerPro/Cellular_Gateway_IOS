...
@interface MKCKFilterByOtherController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKFilterByOtherModel *dataModel;

@end

@implementation MKCKFilterByOtherController

- (void)dealloc {
    NSLog(@"MKCKFilterByOtherController销毁");
}
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        ...
        MKCKFilterRawAdvDataModel *model = [[MKCKFilterRawAdvDataModel alloc] init];
...
- (mk_ck_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_ck_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_ck_filterByOther_AB;
        }
        return mk_ck_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_ck_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_ck_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_ck_filterByOther_AOrBOrC;
        }
    }
    return mk_ck_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKFilterByOtherController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
