...
@interface MKCKFilterByBXPSController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCKFilterByBXPSModel *dataModel;

@end

@implementation MKCKFilterByBXPSController

- (void)dealloc {
    NSLog(@"MKCKFilterByBXPSController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"BXP-Sensor";
    [self.rightButton setImage:LOADICON(@"MKGatewayFour", @"MKCKFilterByBXPSController", @"ck_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCKFilterByBXPSModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCKFilterByBXPSModel alloc] init];
    }
    return _dataModel;
}
