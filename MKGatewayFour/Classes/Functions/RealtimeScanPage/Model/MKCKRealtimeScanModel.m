...
@interface MKCKRealtimeScanModel ()
...
@implementation MKCKRealtimeScanModel
...
- (BOOL)readInterval {
    ...
    [MKCKInterface ck_readRealtimeScanPeriodicReportIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configInterval {
    __block BOOL success = NO;
    [MKCKInterface ck_configRealtimeScanPeriodicReportInterval:[self.interval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (void)operationFailedBlockWithMsg:(NSString *)msg block:(void (^)(NSError *error))block {
    moko_dispatch_main_safe(^{
        NSError *error = [[NSError alloc] initWithDomain:@"ScanReportModeParams"
                                                    code:-999
...
- (dispatch_queue_t)readQueue {
    if (!_readQueue) {
        _readQueue = dispatch_queue_create("ScanReportModeQueue", DISPATCH_QUEUE_SERIAL);
    }
    return _readQueue;
}
