...
@implementation MKCKExcelDataManager

+ (void)exportAppExcel:(id <MKCKExcelAppProtocol>)protocol
              sucBlock:(void(^)(void))sucBlock
           failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKCKExcelAppProtocol)]) {
...
+ (void)exportDeviceExcel:(id <MKCKExcelDeviceProtocol>)protocol
                 sucBlock:(void(^)(void))sucBlock
              failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKCKExcelDeviceProtocol)]) {
        [self operationFailedBlockWithMsg:@"Params Error" block:failedBlock];
        return;
    }
...
+ (BOOL)checkExcelAppProtocol:(id <MKCKExcelAppProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKCKExcelAppProtocol)]) {
        return NO;
    }
...
+ (BOOL)checkExcelDeviceProtocol:(id <MKCKExcelDeviceProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKCKExcelDeviceProtocol)]) {
        return NO;
    }
    if (!ValidStr(protocol.host) || protocol.host.length > 64 || ![protocol.host isAsciiString]) {
