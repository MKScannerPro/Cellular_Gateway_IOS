...
/**
 线程ID
 */
@property (nonatomic, assign)mk_ck_taskOperationID operationID;
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKCKTaskAdopter parseReadDataWithCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKCKTaskAdopter parseWriteDataWithCharacteristic:characteristic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_ck_taskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_ck_defaultTaskOperationID || operationID != self.operationID) {
        return;
    }
    ...
    //分帧数据
    NSString *totalNum = returnData[mk_ck_totalNumKey];
    if (MKValidStr(totalNum)) {
        //存在分帧数据
        self.receiveTimerCount = 0;
        NSData *data = returnData[mk_ck_contentKey];
        if (data) {
